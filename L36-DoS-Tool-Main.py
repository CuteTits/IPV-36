...
from scapy.all import IP, UDP, TCP, RandShort, send  # Fixed import
...
(!) Install if required, run as administrator.
(?) This tool is to only be used for system and DDoS Mitigation testing, unauthorized use may lead to legal consequences.

Before  running, make sure to install:
bash/ pip install beautifulsoup4
bash/ pip install PySocks
bash/ pip install scapy
or Astraeainstall.py

For Win, please install npcap at https://github.com/nmap/npcap

while utilizing tool, remain on secured network or VPN.
...
def starturl():  
    ...
    try:
        ...
        print("ValueError:Please_Try_Again")
        starturl()
...
0 = HTTP / Hypertext Transfer Protocol. Websites, Web Applications, Content Delivery, APIs.
	HTTP packet request flooding.
		
1 = TCP / Transmission Control Protocol. Web Servers, Email Servers, Online Servers. 
	TCP Syn packet flooding. Exploit 3-Way Handshake to send connection requests to a server.
		
2  = UDP /	User Datagram Protocol. Networks, Routers, Firewalls, Online Servers.
	UDP Pacckets Flooding. Connectionless, non-traceable.
...
def proxylist():
	global proxies
	out_file = str(input('''----
Enter the proxylist filename/path 
					  
enter = default (proxy.txt)
